web page link: https://reactRandomNum.ccbp.tech

In this assignment let's build a **Random Number Generator** by applying the
...
- When **Generate** button is clicked, a random number should be generated in
  the range of 0 to 100 and displayed.
...
- You can use `Math.random()` function to get a random number (float value) in
  range 0 to less than 1 (0 <= randomNumber < 1).

  ```
  Math.random()
  ```

- You can use `Math.ceil()` function always rounds a **number up to the next
  largest integer**.
